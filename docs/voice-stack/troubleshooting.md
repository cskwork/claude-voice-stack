...
| Korean transcripts poor or gibberish | Make sure `VOICE_STT=whisper-mlx` (Parakeet has no Korean). Keep `VOICE_STT_LANGUAGE` empty: pinning `ko` turns English speech into Korean text. |
| Korean comes out translated into English | You are on `VOICE_STT=mlx-audio-whisper` with a small model: speech-to-speech 1.0.0 loads the large-v3 tokenizer for repo ids it does not know, so decoding breaks. Use `whisper-mlx` (default). |
